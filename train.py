...
tf.compat.v1.flags.DEFINE_integer("num_epochs", 5, "Number of training epochs (Default: 100)")
...
tf.compat.v1.flags.DEFINE_float("dropout_keep_prob", 0.3, "Dropout keep probability (Default: 0.5)")
...
def train(x_train_head, x_train_body, x_train_image, y_train, modelfolder, boarddir, a, b):
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            #layer.load_weights("checkpoint_folder")
...
def main(_):
    ...
    x_head = np.load('inputs/politifact_embeddings/headline_vectors_30x300.npy')
    x_body = np.load('inputs/politifact_embeddings/text_vectors_100x300.npy')
    x_image = np.load('inputs/politifact_embeddings/caption_vectors_20x300.npy')
    y = np.load('inputs/politifact_embeddings/y_array.npy')
    ...
    modelfolder = './checkpoints_politifact/iteration'
    log = 'saved/logs'

    train(x_head_train, x_body_train, x_image_train, y_train, modelfolder, log, a, b)

def func(a1,b1):
  print("alpha= {} beta= {}".format(b1,b1))
  tf.compat.v1.app.run(argv=[a1,b1])
